                                     ...
                                     QStringList & searchWords, QRegExp & regexp )
{
  ...
  auto findLinks = [ & ]( const QString & word )
  {
    ...
    vector< BtreeIndexing::WordArticleLink > links =
      ftsIndex.findArticles( gd::removeTailingZero( word ), ignoreDiacritics );
                                             ...
                                             QRegExp & regexp )
{
  ...
  if( !wordsList.empty() )
  {
    ...
    {
      QSet< uint32_t > tmp;
      vector< BtreeIndexing::WordArticleLink > links = ftsIndex.findArticles( gd::removeTailingZero( word ) );
      for( unsigned x = 0; x < links.size(); x++ )
      {
        if( Utils::AtomicInt::loadAcquire( isCancelled ) )
        {
