...
vector< WordArticleLink > BtreeIndex::findArticles( wstring const & word1, bool ignoreDiacritics )
{
  //First trim ending zero
  wstring word = gd::removeTailingZero( word1 );
...
void IndexedWords::addWord( wstring const & word1, uint32_t articleOffset, unsigned int maxHeadwordSize )
{
  wstring const & word = gd::removeTailingZero( word1 );
...
void IndexedWords::addSingleWord( wstring const & word1, uint32_t articleOffset )
{
  wstring const & word = gd::removeTailingZero( word1 );
  wstring folded = Folding::apply( word );
  if( folded.empty() )
